...
def find_approximate_normal(
    points, decay=0.99, tol=1e-5, margin=0, max_steps=1000, rng=None, **kwargs
):
...
def get_nearest_neighbors(target_idx, shape_dict, N_shells=3, surface_only=True, **kwargs):
...
def add_adsorbate(
    ...
    **kwargs,
    ...
    if use_sampling:
        surface_ind, shape_dict = alpha_shape_alg_3D_with_sampling(
            points=volume.atoms,
        ...
        surface_ind, shape_dict = alpha_shape_alg_3D(
            points=volume.atoms, probe_radius=probe_radius, return_alpha_shape=True
        )
    ...
    for key, val in filters.items():
        if "element" in key:
            ...
            tmp_mask = reduce(
                lambda x, y: np.logical_or(x, y), [volume.species == x for x in elements]
            )
            ...
            print(
                "Invalid filter key provided. Must be one of {elementXX, maskXX, indicesXX, spatialXX}."
            )
